...
//JDBC to create all of our SQL statments for Transfers

    @Autowired//used to scan the components in the current package and its sub-packages
    ...
    public String sendTransfer(int userFrom, int userTo, BigDecimal amount) {
        ...
        if (amount.compareTo(accountDAO.getBalance(userFrom)) == -1 && amount.compareTo(new BigDecimal(0)) == 1) {
           // int accountFrom = accountDAO.findAccountById(userFrom).getAccountId();
          //  int accountTo = accountDAO.findAccountById(userTo).getAccountId();
            String sql = "INSERT INTO transfers (transfer_type_id, transfer_status_id, account_from, account_to, amount) " +
                    "VALUES (2, 2, ?, ?, ?)";
            jdbcTemplate.update(sql, userFrom + 1000, userTo + 1000, amount);
            accountDAO.addToBalance(amount, userTo + 1000);
            accountDAO.subtractFromBalance(amount, userFrom + 1000);
    ...
    public List<Transfers> getPendingRequests(int userId) {
        ...
        SqlRowSet results = jdbcTemplate.queryForRowSet(sql, userId + 1000, userId + 1000);
        while (results.next()) {
            Transfers transfer = mapRowToTransfer(results);
            output.add(transfer);
        }
    ...
    private Transfers mapRowToTransfer(SqlRowSet results) {
        ...
        try {
            ...
            System.out.println("Unique catch");
        }
        try {
